...
if (isset($_POST["send_register"])) {
   ...
   try {
      ...
      $stmt_ins_stat = $conn->prepare("INSERT INTO stato_presenze (id_lezione, id_corsista, stato, orario_variazione) 
                                             VALUES (?, ?, ?, ?)");
      // updates the presences alredy sents
      $stmt_up_stat = $conn->prepare("UPDATE stato_presenze SET stato = ?, orario_variazione = ? 
                                             WHERE id_corsista = ? AND id_lezione = ?");

      // inserts a new absence in the register lesson
      $stmt_ins_reg = $conn->prepare("INSERT INTO registro_assenze (id_lezione, id_corsista, ora_inizio, ora_fine, note) 
                                             VALUES (?, ?, ?, ?, ?)");
      // updates the end time of a previously saved absence
      $stmt_up_reg = $conn->prepare("UPDATE registro_assenze SET ora_fine = ? 
                                             WHERE id_corsista = ? AND id_lezione = ? AND ora_fine = ?");
      ...
      while($count <= $student_number){
         ...
         // if the first presences state has already been sent, saves only the variations
         } else {
